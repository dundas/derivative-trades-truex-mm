...
| `REFERENCE_MARKOUT_POLL_INTERVAL_MS` | `1000` | Sampling/claim cadence |
...
3. Verify the effective image/config, PostgreSQL table/index creation, service health, FIX logon,
   acknowledged two-sided quotes, and no added rejects or quote gaps.
4. Capture `EXPLAIN (ANALYZE, BUFFERS)` and timings for the observation selector and retention query
   on production PostgreSQL before high-volume collection.
5. Set `REFERENCE_MARKOUT_ENABLED=true`, recreate only the market-maker service, and verify collector
   status plus increasing observation/decision/work counts without quote-health regression.
6. After eligible fills mature, run the bounded coverage audit and verify explicit 1/5/60-minute
   outcomes. Do not promote a strategy from empty, pending, candle-only, or malformed evidence.
