...
- Production requires operator-approved, non-secret values for `MM_NORMAL_QUOTE_LEVELS`, `MM_BASE_QUOTE_SIZE_BTC`, `MM_FALLBACK_BASE_SPREAD_BPS`, `MM_CONTRACT_MAX_QUOTE_SPREAD_BPS`, `MM_MIN_LIVE_QUOTE_WIDTH_BPS`, `MM_CONTRACT_REQUIRED_LEVELS_PER_SIDE`, and `MM_CONTRACT_ORDER_STATE_MAX_AGE_MS`. The supplied example values are placeholders; Compose rejects absent values and the service rejects contradictory values before it begins operating.
- Spread values are full bid-to-ask widths in basis points. `MM_MIN_LIVE_QUOTE_WIDTH_BPS` must be positive and no larger than the contractual ceiling. Mirror and fallback quote pairs are widened around their reference mid to meet that floor; normal, defensive, and fallback widths must not exceed the ceiling.
...
- Tick rounding that makes the floor and ceiling incompatible suppresses the pair before reconciliation. At the final send boundary, live own-order state is checked again: the widest relevant opposite protects the ceiling and the nearest relevant opposite protects the minimum displayed width. No replacement cancel or new order is sent for an incompatible pair.
...
- When `shadowTakeMode=true`, `evaluateShadowTake()` still runs, but `_prepareTakerQuote()` returns `null` before `allowTakerOrders` is checked. The transport-adjacent `_sendNewOrder()` boundary independently rejects direct taker calls before capital reservation, local order mutation, or FIX dispatch. This keeps the shadow canary non-executable even if a future internal caller bypasses normal preparation.
- The deployed observer canary combines `shadowTakeMode=true` with `MM_QUOTE_DISPATCH_MODE=observe`; both independent gates preserve zero-side-effect taker behavior. Focused coverage exercises the combined configuration and the shadow-only fallback, while `scripts/smoke-shadow-take.ts` proves a real would-take evaluation emits zero FIX messages.
