...
15. **(SO-14)** After every PR push, use the exact reviewer solicitation lines from `bun brain/scripts/pr-review-loop.ts remind`; re-post them if no review comments arrive within 5 minutes. Never merge on CI-only.
...
4. **Gate chain** (in order):
   ...
   - `4d` `/pr-review-loop` — solicit every reviewer from `bun brain/scripts/pr-review-loop.ts remind` after every push and again after 5 minutes of silence (SO-14)
