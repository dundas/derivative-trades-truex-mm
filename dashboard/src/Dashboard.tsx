...
export default function Dashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      // Check for any 401 — if health fetch failed with unauthorized, log out
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ ...valueStyle, color: healthColor }}>
                {health?.status ?? '—'}
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: 4, padding: 12, maxHeight: 300, overflowY: 'auto' }}>
          {logs.length === 0 ? (
            ...
            logs.map((line, i) => (
              <div key={i} style={{ color: '#8b949e', fontSize: 11, lineHeight: '1.6', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                {line}
              </div>
            ))
          )}
