...
export function duplicateSession(originalSession, options = {}) {
  ...
  let duplicatedSession = JSON.parse(JSON.stringify(originalSession));
  ...
  // Apply caller overrides without allowing prototype mutation through parsed input.
  const forbiddenOverrideKeys = ['__proto__', 'prototype', 'constructor'];
  const unsafeOverrideKey = forbiddenOverrideKeys.find(key =>
    Object.prototype.hasOwnProperty.call(overrides, key)
  );
  if (unsafeOverrideKey) {
    throw new Error(`Unsafe session override key: ${unsafeOverrideKey}`);
  }
  duplicatedSession = { ...duplicatedSession, ...overrides };
