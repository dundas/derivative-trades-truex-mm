...
export class OrderManager {
  ...
  async getAll() {
    ...
    try {
      ...
      this.logger.debug(`[OrderManager.getAll] Raw HGETALL result: ${JSON.stringify(rawResult)}`);
  ...
  async add(order) {
    try {
      ...
      if (order.pricingMetadata && !validatedOrder.pricingMetadata) {
        ...
        this.logger.debug(`[PRICING_METADATA_DEBUG] Pricing metadata preserved through validation for order ${validatedOrder.id}`);
      }
      ...
      try {
          ...
          this.logger.debug(`[PRICING_METADATA_DEBUG] About to stringify order ${validatedOrder.id}:`, {
              hasPricingMetadata: !!validatedOrder.pricingMetadata,
              pricingMetadataKeys: validatedOrder.pricingMetadata ? Object.keys(validatedOrder.pricingMetadata) : 'NONE',
          ...
          if (validatedOrder.pricingMetadata && !parsedBack.pricingMetadata) {
              ...
              this.logger.debug(`[PRICING_METADATA_DEBUG] Pricing metadata preserved through JSON stringify/parse for order ${validatedOrder.id}`);
          }
