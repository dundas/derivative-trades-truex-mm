...
export class PostgreSQLAdapter {
  ...
  async initialize() {
    ...
    try {
      ...
      // Disable SSL for local connections or when sslmode=disable is specified
      const isLocal = /localhost|127\.0\.0\.1/.test(this.connectionString);
      const sslDisabled = /sslmode=disable/.test(this.connectionString);
      const sslConfig = (isLocal || sslDisabled) ? false : { rejectUnauthorized: false };
