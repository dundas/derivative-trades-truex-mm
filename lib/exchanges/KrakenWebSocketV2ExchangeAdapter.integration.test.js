...
import { OrderManager } from '../redis-backend-api/order-manager.js';
import { FillManager } from '../redis-backend-api/fill-manager.js';
import { jest, test, describe, it, expect, beforeEach, afterEach, beforeAll, afterAll } from 'bun:test';
...
const RUN_LIVE_KRAKEN_TESTS = process.env.RUN_LIVE_KRAKEN_TESTS === '1';
const describeLiveKraken = RUN_LIVE_KRAKEN_TESTS ? describe : describe.skip;
...
describeLiveKraken('KrakenWebSocketV2ExchangeAdapter - Integration Tests', () => {
  let adapter;
...
// Live connectivity test; opt-in only so the default suite stays deterministic.
describeLiveKraken('Kraken API Connectivity Test in Jest Environment', () => {
  beforeAll(() => {
