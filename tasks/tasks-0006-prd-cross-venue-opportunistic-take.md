...
- [ ] **2.0a PYUSD/USD basis feed** — smoke: basis value populates (outbound surface, NOT skippable)
  ...
  - [ ] 2.2 Use 0.3 findings to stand up a real PYUSD-USD source: **prefer Kraken public REST ticker `PYUSDUSD`** (live on 2026-06-18) with its own freshness/error handling; do **not** use a second `CoinbaseWsIngest` instance unless Coinbase relists `PYUSD-USD`
...
- [ ] **3.0 Shadow opportunity detection + "would-take" logging** (requires 0.0 + 1.0 + 2.0b) — smoke: mandatory (`scripts/smoke-shadow-take.ts`)
  ...
  - [ ] 3.9 Structured `would-take` log: side, size, truexPrice, rawEdgeBps, basisAdjEdgeBps, pyusdUsd, coinbaseFresh, truexTapeAgeS, dedupKey, suppressReason(if any)
