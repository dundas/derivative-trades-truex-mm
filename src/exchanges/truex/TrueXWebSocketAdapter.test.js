...
describe('TrueXWebSocketAdapter', () => {
    ...
    describe('Constructor', () => {
        ...
        it('should initialize in paper mode without connecting', () => {
            const paperAdapter = new TrueXWebSocketAdapter({
                ...defaultConfig,
                paperMode: true,
                logger: mockLogger
            });
            
            expect(paperAdapter.paperMode).toBe(true);
            expect(WebSocket).not.toHaveBeenCalled();
            paperAdapter.cleanup();
        });
    });
    ...
    describe('Paper Trading', () => {
        ...
        beforeEach(() => {
            paperAdapter = new TrueXWebSocketAdapter({
                ...defaultConfig,
                paperMode: true,
                logger: mockLogger
            });
        ...
        it('should create paper order', async () => {
            const orderDetails = {
                symbol: 'BTC/USD',
                side: 'buy',
                type: 'limit',
            ...
            const order = await paperAdapter.createOrder(orderDetails);
            ...
            expect(order.id).toMatch(/^paper-\d+-\d+$/);
        });
        
        it('should cancel paper order', async () => {
            const orderDetails = {
                symbol: 'BTC/USD',
                side: 'sell',
                type: 'limit',
            ...
            const order = await paperAdapter.createOrder(orderDetails);
            const canceledOrder = await paperAdapter.cancelOrder(order.id);
            
            expect(canceledOrder.status).toBe('canceled');
            expect(paperAdapter.activeOrders.has(order.id)).toBe(false);
        });
    });
    ...
    describe('Cleanup', () => {
        it('should clean up resources properly', async () => {
            ...
            expect(adapter.subscriptions.channels.size).toBe(0);
