...
export function evaluatePolicy(events, { policy, split, assumptions = {}, referencePrices = [] } = {}) {
  ...
  const warnings = []; if (!validation.length) warnings.push('unsupported-regime:no-validation-events'); if (!observed) warnings.push('unsupported-regime:no-observed-fills'); if (contextCoverage < config.minContextCoverage) warnings.push(`missing-context:coverage=${contextCoverage.toFixed(3)}`); if (missingPolicyVectors) warnings.push(`missing-policy-vector:${missingPolicyVectors}`); if (malformedQuoteVectors) warnings.push(`malformed-policy-vector:${malformedQuoteVectors}`); if (unsupportedQuoteVectors) warnings.push(`unsupported-quote-policy-vector:${unsupportedQuoteVectors}`); if (outcomes.unlinkedFills) warnings.push(`unlinked-fills:${outcomes.unlinkedFills}`); if (Object.values(markouts).some(v => v.unavailable)) warnings.push('markout-unavailable-or-stale');
